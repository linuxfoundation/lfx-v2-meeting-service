...
func NewSMTPService(config SMTPConfig) (*SMTPService, error) {
	...
	// Load HTML template
	htmlTemplate, err := template.New("invitation.html").Funcs(template.FuncMap{
	...
	service.templates.invitationHTML = htmlTemplate

	// Load text template
	textTemplate, err := template.New("invitation.txt").Funcs(template.FuncMap{
		"formatTime":     formatTime,
		"formatDuration": formatDuration,
	...
	service.templates.invitationText = textTemplate
