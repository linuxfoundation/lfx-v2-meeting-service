...
import (
	...
	meetingsvr "github.com/linuxfoundation/lfx-v2-meeting-service/gen/http/meeting/server"
...
func handleHTTPServer(ctx context.Context, u *url.URL, meetingEndpoints *meeting.Endpoints, wg *sync.WaitGroup, errc chan error, dbg bool) {
	...
	// responses.
	var (
		meetingServer *meetingsvr.Server
	)
	{
		eh := errorHandler(ctx)
		meetingServer = meetingsvr.New(meetingEndpoints, mux, dec, enc, eh, nil)
	}

	// Configure the mux.
	meetingsvr.Mount(mux, meetingServer)
	...
	for _, m := range meetingServer.Mounts {
